...
```{r echo =FALSE}
Rdat_path<- "~/Documents/Luftqualitaet/Daten/BW_Rdat/"
load(file.path(Rdat_path,"BW_list_tbl.RData"))
load(file.path(Rdat_path,"stationlist.RData"))
BW_list_tbl
```

19 sampling stations operated by LUBW (Landesanstalt für Umwelt Baden-Wuerttemberg) have been selected for analysis of concentrations of atmospheric constituents.

```{r echo = FALSE}
knitr::kable(Stationsliste)
```

3 sampling stations rural environments, 3 stations located at main streets, 16 sampling urban background immissions. 
