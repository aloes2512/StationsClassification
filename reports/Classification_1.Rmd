...
The objective of this study is to understand the systematic dependence of the concentrations of  atmospheric gases on  variables as the emissions from various sources:  
-- motor vehicles,   
-- power-plants,   
...
Data are measured at 99  sampling stations by LUBW: **Landesanstalt für Umwelt Baden-Württemberg**. These are equipped with 780 instruments for various atmospheric components.
To understand the effects of human emission sources on the immissions, the dependence of  location and the interaction with natural atmospheric components typical stations have been selected.
...
### Location  of selected sampling stations
 ...
 At some stations measurements started as early as 1976. Some typical stations have been selected for analysis of immissions and interaction of constituents. Data can be downloaded from  url:"https://https://udo.lubw.baden-wuerttemberg.de/public". Some missing data have been included from 2 stations operated by the city of Stuttgart "Amt für Umweltschutz".
...
The positions are shown in this map derived from Google Maps:
...
As example of NO2 1-h time series, measured at Stuttgart- Bad Cannstatt at urban background are shown in the plot "NO2-immissions 20 years". The data exhibit a systematic trends suggest periodic dependencies and  are poised by various effects as illustrated with the dots (each representing a  1- h mean) plotted together with red linear regression line over 20 years demonstrating a reduction trend.In addition a systematic annual variation with high values during winter time and low values during the summer month is visible with the plot:

![Stg_Can](./figs/NO2_trend_20_years_Can.png)

 This suggests some systematic dependencies:    
...
--  21 year means  of $NO_2$ are smaller  than $10[μg/m^3]$at rural stations and exceed $40[μg/m^3]$ close to main routes   
...
```
The table is arranged in ascending order of **mean NO2 immissions measurements** calculated from 1-h measurements  recorded and published by LUBW (Landesanstalt für Umwelt Baden Wuerttemberg). The mean is calculated   of all recorded data (21 year interval).   
...
-- at Stuttgart "Am Neckartor" the maximum $mean  NO_2 $ were recorded     
   --- starting in 2003 with annual mean greater $125 [μg/m^3]$ which were reduced to $50 [μg/m^3]$in 2020    
   --- long time trend "Am Neckartor"  indicates levels smaller $40 [μg/m^3]$
...
Mean yearly reductions(chg_py) range from $-5.1[μg/(m^3*y)]$ at the "Hauptstätter Strasse" and no change at all $0 [μg/(m^3*y)]$ at the rural background  station "Schwarzwald-Süd".


### Effect of Heating on $NO_2, NO, O_3

### Mean Temperatures
Temperatures vary within a wide range. To study the dependance of immissions it is necessary to define a method for calculation of a small number of parameters characterizing local temperatures. To visualize various methods have been applied to the 1- h temperature measurements made at Stuttgart Bad Cannstatt:
...
The simplest model is the regression line as reference. As only two parameters ** intersect** and **slope** are sufficient to describe temperature and trends. 
In Germany at temperatures **above 15 C no heating** is necessary. Therefor at these temperatures are no NO2- emissions caused by burning fuel for heating. This can be used to determine the effect of house heating on measured immissions.
